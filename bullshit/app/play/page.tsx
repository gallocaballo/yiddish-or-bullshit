...
import type { ConfidenceLevel, ContentItem, GameMode } from "@/lib/types";
...
function PlayGame() {
  ...
  // Start session on mount — fetch from API, fallback to static pool
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadItems() {
      try {
        const endpoint =
          mode === "daily"
            ? `/api/daily?date=${getTodayDateStr()}`
            : "/api/freeplay";
        const res = await fetch(endpoint);
        if (!res.ok) throw new Error(`API ${res.status}`);
        const { items } = await res.json();
        if (!cancelled) actions.startSession(items, mode);
      } catch {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg space-y-6">
        ...
        {/* Next button during reveal (tap-anywhere also works as shortcut) */}
        {isRevealed && (
          <button
            type="button"
            onClick={(e) => {
            ...
            className="block w-full animate-[fadeIn_200ms_ease-in_500ms_both] rounded-lg bg-text-primary px-6 py-4 text-center text-lg font-semibold text-bg-primary transition-opacity hover:opacity-90"
          >
            Next &rarr;
          </button>
        )}
