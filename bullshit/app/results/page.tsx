...
/** Safe lookup for confidence tier label (guards against old sessionStorage data) */
function tierLabel(confidence: number): string {
  return CONFIDENCE_TIERS[confidence as ConfidenceLevel]?.label ?? "—";
}
...
function ResultsContent() {
  ...
  const handleCopyText = useCallback(async () => {
    ...
    const text = `I scored ${result.totalScore} on today's Bullshit challenge. ${correctCount}/5 correct. Can you spot the fake headlines? playbullshit.com`;
    try {
      ...
      <div className="w-full max-w-lg space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card-bg p-6 text-center shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
            ...
            <div>
              <p className="font-bold text-text-primary">
                {tierLabel(Math.round(result.averageConfidence))}
              </p>
        ...
        {/* Share card (off-screen for capture — positioned off-screen so layout is computed correctly by html2canvas) */}
